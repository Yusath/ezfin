...
const Navbar: React.FC = () => {
  ...
  const NavItem = ({ to, icon: Icon, label, active }: { to: string, icon: any, label: string, active: boolean }) => (
    <Link 
      to={to} 
      className={`flex flex-col items-center justify-center w-full gap-1 pt-2 pb-1 transition-all duration-300 active:scale-90 ${active ? 'text-primary' : 'text-gray-600 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'}`}
    >
