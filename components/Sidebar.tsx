...
const Sidebar: React.FC = () => {
  ...
  const NavItem = ({ to, icon: Icon, label }: { to: string, icon: any, label: string }) => (
    ...
    >
      <Icon size={20} className={isActive(to) ? '' : 'group-hover:scale-110 transition-transform'} />
      <span className="font-semibold text-sm tracking-wide">{label}</span>
    </Link>
  );

  return (
    <aside className="w-64 h-full bg-white dark:bg-[#1C1C1E] border-r border-gray-200 dark:border-white/10 flex flex-col p-6 shadow-xl z-30">
      ...
      <div className="mt-auto pt-8">
        <Link 
          to="/add" 
          className="flex items-center justify-center gap-2 w-full bg-black dark:bg-white dark:text-black text-white py-3.5 rounded-2xl font-bold shadow-lg active:scale-95 transition-all hover:opacity-90"
        >
          <Plus size={20} />
          <span>{t('nav.add')}</span>
        </Link>
