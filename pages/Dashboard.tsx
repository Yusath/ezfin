...
const Dashboard: React.FC<DashboardProps> = ({ user, transactions, onUpdateUser }) => {
  ...
  // Balance should be accumulated from all transactions, not just this month.
  const totalIncomeAllTime = transactions.filter(t => t.type === 'income').reduce((sum, t) => sum + t.totalAmount, 0);
  const totalExpenseAllTime = transactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.totalAmount, 0);
  ...
  const monthlyTransactions = transactions.filter(t => isSameMonth(new Date(t.date), currentMonth));
  const monthlyIncome = monthlyTransactions.filter(t => t.type === 'income').reduce((sum, t) => sum + t.totalAmount, 0);
  const monthlyExpense = monthlyTransactions.filter(t => t.type === 'expense').reduce((sum, t) => sum + t.totalAmount, 0);

  // 3. Recent Transactions (GLOBAL - Sorted by Date)
  // Ensure we sort descending by date. 
  // IMPORTANT: Creating a copy before sorting to avoid mutating props
  ...
  const getChartData = () => {
    ...
    while (cursor <= end) {
        ...
        dataMap.set(key, 0);
        groupBy === 'day' ? cursor.setDate(cursor.getDate() + 1) : cursor.setMonth(cursor.getMonth() + 1);
    }
    ...
    filteredTxs.forEach(t => {
      ...
      if (d >= start && d <= end) {
         ...
         if (dataMap.has(key)) dataMap.set(key, (dataMap.get(key) || 0) + t.totalAmount);
      }
    });
  ...
  const generateNewAvatar = () => {
    // Uses UI Avatars for clean, professional look based on name
  ...
  return (
    <div className="pt-safe px-6 md:px-0 space-y-8 animate-fade-in pb-10">
      
      {/* Header Profile - Mobile Only */}
      <div className="flex items-center justify-between pt-4 md:hidden">
        <button 
          onClick={() => setIsProfileModalOpen(true)}
          className="flex items-center gap-4 group active:opacity-70 transition-opacity"
        >
          <img 
            src={user.avatarUrl} 
            alt="Profile" 
        ...
        <Link 
          to="/settings"
          className="w-10 h-10 flex items-center justify-center rounded-full bg-white dark:bg-[#1C1C1E] shadow-sm text-gray-600 dark:text-gray-300 active:scale-90 transition-all"
        >
      ...
      <div className="hidden md:flex items-center justify-between mb-8">
        ...
        <button 
          onClick={() => setIsProfileModalOpen(true)}
          className="flex items-center gap-3 bg-white dark:bg-[#1C1C1E] px-4 py-2 rounded-xl shadow-sm hover:shadow-md transition-all"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        {/* Hero Card (Balance) */}
        <div className="md:col-span-8 relative overflow-hidden rounded-[2.5rem] p-8 shadow-2xl shadow-blue-500/20 bg-gradient-to-br from-[#007AFF] to-[#5856D6] text-white transform transition-transform hover:scale-[1.01] duration-300">
        ...
        {/* Quick Stats / Info (Desktop Only) */}
        <div className="hidden md:flex md:col-span-4 bg-white dark:bg-[#1C1C1E] rounded-[2.5rem] p-6 shadow-sm border border-gray-100 dark:border-gray-800 flex-col justify-center">
        ...
        {/* Recent List */}
        <div className="md:col-span-4 md:row-span-2 flex flex-col h-full">
           <div className="flex justify-between items-center px-1 mb-4">
            <h3 className="font-bold text-xl text-gray-900 dark:text-white">{t('dash.recent')}</h3>
            <Link to="/history" className="text-sm text-primary font-medium hover:underline">{t('dash.viewAll')}</Link>
          </div>
            ...
            <div className="space-y-3 h-full overflow-y-auto no-scrollbar md:pr-2">
              {recentTransactions.length === 0 ? (
                ...
                recentTransactions.map(tx => (
                  <div key={tx.id} className="flex justify-between items-center bg-white dark:bg-[#1C1C1E] md:bg-gray-50 md:dark:bg-black/20 p-4 rounded-2xl shadow-sm md:shadow-none active:scale-[0.98] transition-transform duration-200 border border-gray-100 dark:border-gray-800 md:border-transparent">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-0.5">{new Date(tx.date).toLocaleDateString('id-ID', {day:'numeric', month:'short'})}</p>
                      </div>
                    </div>
                    <span className={`font-bold text-sm whitespace-nowrap ${tx.type === 'income' ? 'text-green-600' : 'text-red-600'}`}>
                      {tx.type === 'income' ? '+' : '-'} {tx.totalAmount.toLocaleString('id-ID', { notation: 'compact' })}
                    </span>
                  </div>
                ))
              )}
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in w-full">
          <div className="bg-white dark:bg-[#1C1C1E] w-full max-w-md rounded-t-[2rem] sm:rounded-[2rem] p-8 shadow-2xl animate-slide-up max-h-[90vh] overflow-y-auto">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-xl font-bold text-gray-900 dark:text-white">Account & Profile</h3>
              <button onClick={() => setIsProfileModalOpen(false)} className="p-2 bg-gray-100 dark:bg-gray-800 rounded-full text-gray-500 hover:bg-gray-200">
                <X size={20} />
              </button>
            </div>
            ...
            <div className="flex flex-col items-center mb-8">
              <div className="relative cursor-pointer active:scale-95 transition-transform" onClick={generateNewAvatar}>
               ...
               {user.googleEmail ? (
                 ...
                 <div className="space-y-3">
                   ...
                   <button 
                     ...
                     className="w-full bg-white dark:bg-black border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white py-3 rounded-xl font-bold shadow-sm flex items-center justify-center gap-2 hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors"
                   >
                     {isGoogleLoading ? (
            ...
            <div className="space-y-6">
              ...
              <button 
                ...
                className="w-full bg-primary text-white py-4 rounded-2xl font-bold text-lg shadow-lg shadow-blue-500/30 active:scale-95 transition-transform hover:bg-blue-600"
              >
                Save Changes
              </button>
            </div>
