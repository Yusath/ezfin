...
import { TrendingUp, TrendingDown, Wallet, X, Settings as SettingsIcon, LogIn, CheckCircle, RefreshCcw, PieChart, Activity } from 'lucide-react';
  ...
  return (
    <div className="pt-safe md:px-0 space-y-6 md:space-y-8 page-transition pb-28 md:pb-10 w-full overflow-x-hidden">
      ...
      <div className="flex items-center justify-between pt-4 px-4 md:hidden">
        ...
        >
          <img 
            src={user.avatarUrl} 
            alt="Profile" 
            className="w-12 h-12 rounded-full object-cover shadow-sm border-2 border-white dark:border-card-dark"
          />
        ...
        <Link 
          to="/settings"
          className="w-10 h-10 flex items-center justify-center rounded-full bg-white dark:bg-white/10 shadow-sm text-gray-600 dark:text-gray-300 ios-touch-target"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 w-full">
          ...
          <div className="relative z-10 flex flex-col items-center md:items-start text-center md:text-left">
            ...
            <div className="grid grid-cols-2 gap-3 md:gap-4 w-full md:max-w-md">
              <div className="bg-white/20 backdrop-blur-md rounded-2xl p-3 md:p-4 flex flex-col items-center md:items-start border border-white/10 hover:bg-white/30 transition-colors min-w-0">
                ...
                <span className="font-bold text-lg md:text-xl truncate w-full">Rp {monthlyIncome.toLocaleString('id-ID')}</span>
              </div>
              <div className="bg-white/20 backdrop-blur-md rounded-2xl p-3 md:p-4 flex flex-col items-center md:items-start border border-white/10 hover:bg-white/30 transition-colors min-w-0">
                <div className="flex items-center gap-1.5 md:gap-2 mb-1 justify-center md:justify-start w-full">
                  <div className="p-1 md:p-1.5 bg-white/20 rounded-full shrink-0">
                ...
                <span className="font-bold text-lg md:text-xl truncate w-full">Rp {monthlyExpense.toLocaleString('id-ID')}</span>
              </div>
            </div>
          </div>
        ...
        {/* Quick Stats / Info - Refactored for Grid on Mobile */}
        <div className="col-span-12 md:col-span-4 flex flex-col h-full mx-4 md:mx-0">
           <div className="bg-card-light dark:bg-card-dark rounded-[2rem] p-6 shadow-sm border border-gray-100 dark:border-white/5 h-full flex flex-col justify-center">
             ...
             <div className="grid grid-cols-2 md:grid-cols-1 gap-3 md:gap-4 h-full">
               <div className="flex flex-col justify-center items-center md:items-start p-4 bg-surface dark:bg-white/5 rounded-2xl md:flex-row md:justify-between">
                 <div className="text-center md:text-left">
                    <span className="text-xs font-medium text-gray-500 block mb-1">{t('dash.txMonth')}</span>
                    <span className="font-bold text-gray-900 dark:text-white text-2xl md:text-lg">{monthlyTransactions.length}</span>
                 </div>
               ...
               <div className="flex flex-col justify-center items-center md:items-start p-4 bg-surface dark:bg-white/5 rounded-2xl md:flex-row md:justify-between">
                 <div className="text-center md:text-left w-full">
                    <span className="text-xs font-medium text-gray-500 block mb-1">{t('dash.avgSpend')}</span>
                    <span className="font-bold text-gray-900 dark:text-white text-lg md:text-lg truncate block">
                       Rp {Math.round((monthlyExpense || 0) / (new Date().getDate() || 1)).toLocaleString('id-ID')}
                    </span>
                 </div>
        ...
        <div className="col-span-12 md:col-span-8 space-y-4 mx-4 md:mx-0">
          ...
          <div className="h-64 md:h-80 w-full bg-card-light dark:bg-card-dark rounded-[2rem] p-6 shadow-sm border border-gray-100 dark:border-white/5">
            {hasChartData ? (
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  />
                  <Tooltip 
                    contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.8)', borderRadius: '12px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.1)', backdropFilter: 'blur(10px)' }}
            ...
            <div className="space-y-3 h-full overflow-y-auto no-scrollbar md:pr-2 pb-4 md:pb-0">
              {recentTransactions.length === 0 ? (
                <div className="flex flex-col items-center justify-center h-48 text-center rounded-3xl border border-dashed border-gray-200 dark:border-white/10 m-2">
                   ...
                   <p className="text-sm text-gray-400 font-medium">{t('dash.noTx')}</p>
                </div>
              ) : (
                recentTransactions.map((tx, idx) => {
                  ...
                  return (
                    <div 
                      key={tx.id} 
                      className="flex justify-between items-center bg-white dark:bg-card-dark md:bg-gray-50 md:dark:bg-white/5 p-4 rounded-2xl shadow-sm md:shadow-none ios-touch-target border border-gray-100 dark:border-white/5 md:border-transparent animate-slide-in-right group"
                      style={{ animationDelay: `${idx * 0.05}s` }}
                    >
                      <div className="flex items-center gap-4">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center group-active:scale-95 transition-transform ${tx.type === 'income' ? 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400' : 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'}`}>
                          {tx.type === 'income' ? <TrendingUp size={20} /> : <TrendingDown size={20} />}
                        </div>
                        <div>
                          <p className="font-bold text-gray-900 dark:text-white text-sm line-clamp-1">{tx.storeName}</p>
                          <div className="flex gap-2 text-xs text-gray-400 mt-0.5 font-medium">
      ...
      {isProfileModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-end sm:items-center justify-center bg-black/40 backdrop-blur-sm animate-fade-in w-full">
          <div className="bg-card-light dark:bg-card-dark w-full max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] p-8 shadow-2xl animate-ios-slide-up max-h-[90vh] overflow-y-auto page-slide-up">
            ...
            <div className="mb-8 p-4 bg-gray-50 dark:bg-white/5 rounded-3xl border border-gray-100 dark:border-white/5">
               ...
               {user.googleEmail ? (
                 ...
                 <div className="space-y-3">
                   ...
                   <button 
                     ...
                     className="w-full bg-card-light dark:bg-black border border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white py-3.5 rounded-2xl font-bold shadow-sm flex items-center justify-center gap-2 hover:bg-gray-50 dark:hover:bg-gray-900 transition-colors ios-touch-target"
                   >
                     {isGoogleLoading ? (
            ...
            <div className="space-y-6">
              <div>
                <label className="text-xs font-bold text-gray-400 uppercase ml-3 mb-2 block">Display Name</label>
                <input 
                  type="text" 
                  value={editName}
                  onChange={(e) => setEditName(e.target.value)}
                  className="w-full bg-gray-100 dark:bg-white/10 border-none rounded-2xl px-5 py-4 font-semibold text-gray-900 dark:text-white focus:ring-2 focus:ring-primary/50"
                  placeholder="Nama Lengkap"
                />
              </div>
              <button 
                onClick={handleSaveProfile}
                className="w-full bg-primary text-white py-4 rounded-2xl font-bold text-lg shadow-lg shadow-blue-500/30 ios-touch-target hover:bg-blue-600 active:scale-95 transition-all"
              >
                Save Changes
              </button>
            </div>
