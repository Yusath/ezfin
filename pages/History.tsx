  ...
  return (
    <div className="pt-safe min-h-screen bg-[#F2F2F7] dark:bg-black page-transition">
      ...
      <div className="px-4 mb-4">
        <div className="bg-white dark:bg-[#1C1C1E] rounded-2xl p-3 flex items-center shadow-sm mb-4 border border-gray-100 dark:border-white/5 transition-colors focus-within:ring-2 focus-within:ring-primary/20">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-2">
           {['all', 'expense', 'income'].map(f => (
             <button
               ...
               className={`px-5 py-2 rounded-full text-xs font-bold capitalize transition-all ios-touch-target ${filter === f ? 'bg-primary text-white shadow-lg shadow-blue-500/20' : 'bg-white dark:bg-[#1C1C1E] text-gray-500 border border-gray-100 dark:border-white/5'}`}
             >
      ...
      <div className="px-4 pb-32 space-y-6">
        {Object.entries(groupedTransactions).map(([date, txs], groupIdx) => (
          <div key={date} className="animate-slide-in-right" style={{ animationDelay: `${groupIdx * 0.05}s` }}>
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wide ml-3 mb-2">{date}</h3>
            <div className="bg-white dark:bg-[#1C1C1E] rounded-[1.5rem] overflow-hidden shadow-sm border border-gray-100 dark:border-white/5">
              {txs.map((tx, idx) => (
                <div key={tx.id}>
                  <div className="p-4 flex items-center justify-between active:bg-gray-50 dark:active:bg-white/5 transition-colors ios-touch-target">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="font-bold text-sm text-gray-900 dark:text-white leading-tight mb-0.5">{tx.storeName}</p>
                        <p className="text-xs text-gray-400 font-medium">{tx.category} • {new Date(tx.date).toLocaleTimeString('id-ID', {hour:'2-digit', minute:'2-digit'})}</p>
                      </div>
                    </div>
                  ...
                  {idx < txs.length - 1 && <hr className="border-gray-100 dark:border-white/5 ml-16" />}
                </div>
              ))}
            </div>
          </div>
        ...
        {Object.keys(groupedTransactions).length === 0 && (
           <div className="text-center py-20 text-gray-400 flex flex-col items-center">
              ...
              <p className="text-sm font-medium">{t('hist.noFound')}</p>
           </div>
        )}
      </div>
    </div>
