...
const Stats: React.FC<StatsProps> = ({ transactions, user }) => {
  ...
  return (
    <div className="p-6 pb-28 md:pb-10 min-h-screen space-y-6 bg-[#F2F2F7] dark:bg-black page-transition pt-safe">
      <div className="pb-2">
        <h1 className="text-3xl font-extrabold text-black dark:text-white tracking-tight">{t('stats.title')}</h1>
      </div>

      {/* Pie Chart Card */}
      <div className="bg-white dark:bg-[#1C1C1E] p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-white/5">
        <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-6 text-center">{t('stats.dist')}</h3>
        <div className="h-64 w-full">
          {chartData.length > 0 ? (
            ...
            <div className="h-full flex flex-col items-center justify-center text-gray-400 space-y-2">
              ...
              <p className="text-sm font-medium opacity-50">{t('stats.noData')}</p>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Top Categories List */}
      <div className="bg-white dark:bg-[#1C1C1E] p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-white/5">
         <h3 className="font-bold mb-6 dark:text-white text-lg">{t('stats.details')}</h3>
         <div className="space-y-4">
            {chartData.sort((a, b) => b.value - a.value).map((item, idx) => (
              <div key={item.name} className="flex justify-between items-center animate-slide-in-right p-2 rounded-xl hover:bg-gray-50 dark:hover:bg-white/5 transition-colors" style={{ animationDelay: `${idx * 0.1}s` }}>
                ...
                <span className="text-sm font-bold text-gray-900 dark:text-white">Rp {item.value.toLocaleString()}</span>
              </div>
            ))}
         </div>
      </div>
      ...
      >
        ...
        <div className="text-left">
          ...
          <p className="text-xs opacity-80 text-indigo-100 font-medium">{t('stats.advisor.desc')}</p>
        </div>
      </button>
      ...
      {isAdvisorOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-md w-full animate-ios-fade-in">
          <div className="bg-white dark:bg-[#1C1C1E] w-full max-w-md rounded-t-[2.5rem] sm:rounded-[2.5rem] shadow-2xl overflow-hidden page-slide-up max-h-[80vh] flex flex-col m-0 sm:m-4">
            <div className="bg-gradient-to-r from-indigo-600 to-purple-600 p-6 flex justify-between items-center text-white shrink-0">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-bold text-lg">FinBot Advisor</h3>
              </div>
            ...
            <div className="p-6 overflow-y-auto min-h-[200px]">
              {loadingAdvice ? (
                <div className="flex flex-col items-center justify-center py-12 space-y-6">
                  ...
                  <p className="text-sm font-medium text-gray-500 animate-pulse">Sedang menganalisis keuanganmu...</p>
                </div>
              ) : (
                <div className="prose dark:prose-invert text-sm animate-ios-fade-in">
                   <div className="bg-indigo-50 dark:bg-indigo-900/20 p-4 rounded-2xl mb-4">
                      <p className="whitespace-pre-line text-gray-800 dark:text-gray-200 leading-relaxed font-medium">
                        {advice}
                      </p>
                   </div>
